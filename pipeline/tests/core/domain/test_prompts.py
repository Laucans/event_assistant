...
def test_the_preamble_demands_the_markers_the_parser_reads():
    """Les deux moities d'un meme contrat, et rien ne les reliait.

    Le preambule exige `AGENT_LOOP_OK:` en fin de reponse, `read_markers` le
    lit, et les deux ecrivent la chaine en dur a deux dossiers d'ecart.
    Renommer un marqueur laissait le preambule en demander un autre — sans
    erreur, sans test rouge : le round lisait simplement « le stage n'a rien
    repondu » a chaque fois.
    """
    said = prompts.preamble(BRANCH)
    assert stage_result.OK_MARKER in said
    assert stage_result.STOP_MARKER in said
