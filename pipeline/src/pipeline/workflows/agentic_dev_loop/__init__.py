...
`stages/` porte la sequence — la table `PIPELINE`, le design metier du round.
Ce qui fait le travail est dans `internals/` : `gates` ce qu'un noeud exige et
doit obtenir, `board` le cote lecture des issues, `loop` la repetition des
rounds, et `state` ce qui est persiste.
"""
